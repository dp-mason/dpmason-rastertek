...
bool TextureShaderClass::SetShaderParameters(float* worldMatrix, float* viewMatrix, float* projectionMatrix)
{
    ...
    // The location in the pixel shader for the shaderTexture variable is obtained here and then the texture unit is set. Since we only have one texture, we will use texture unit zero for it when we set it using glUniform1i.
